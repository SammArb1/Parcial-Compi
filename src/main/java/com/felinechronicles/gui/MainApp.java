        ...
        switch (mission) {
            case 1: {
                ...
                if (rows > 50 || cols > 50) {
                    ...
                    // Extraer los datos para dibujar minas, S, N y el camino
                    ...
                    drawingPanel.drawGridExtended(rows, cols, mines, new Point(startCol, startRow), new Point(destCol, destRow), path); 
                }
                ...
                for (int caseNum = 1; caseNum <= t; caseNum++) {
                    ...
                    if (caseNum == 1) {
                        if (n2 > 60) {
                            ...
                            drawingPanel.setGraphData(n2, allEdges, sol, "Ruta de Pola");
                        }
                    }
                    sbRes.append(dijkstra.findShortestPath(caseNum, start2, dest2)).append("\n");
                }
                ...
                for (int caseNum = 1; caseNum <= t; caseNum++) {
                    ...
                    if (caseNum == 1) {
                        if (n3 > 60) {
                            ...
                            drawingPanel.setGraphData(n3, allEdges, allEdges, "Minerva vs Limón");
                        }
                    }

                    sbRes.append(maximizer.solveWithBellmanFord(caseNum, start3, dest3)).append("\n");
                    
                    if (caseNum == 1) {
                        long[][] matrix = maximizer.runFloydWarshall();
                        StringBuilder sb = new StringBuilder("Matriz Cruzada N x N (Floyd-Warshall - Primer Caso):\n\n");
                        for (int i = 0; i < n3; i++) {
                            for (int j = 0; j < n3; j++) {
                                long val = matrix[i][j];
                ...
                for (int caseNum = 1; caseNum <= t; caseNum++) {
                    ...
                    if (caseNum == 1) {
                        if (n4 > 100 || c4 > 300) {
                            ...
                            drawingPanel.setGraphData(n4, allEdges, mst, "Red de Kruskal (MST)");
                        }
                    }
                    sbRes.append(kruskal.solve(caseNum)).append("\n");
                }
                outputArea.setText(sbRes.toString());
                bottomTabs.setSelectedIndex(0);
                break;
...
class DrawingPanel extends JPanel {
    ...
    public void drawGridExtended(int rows, int cols, boolean[][] mines, Point start, Point dest, List<Point> path) {
    ...
    public void setGraphData(int nodes, List<EdgeUI> allEdges, List<EdgeUI> solutionEdges, String title) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (type == 0) {
            ...
            g2.drawString(message, (w - msgW) / 2, h / 2);
        } else if (type == 1) { 
            // Dibujo de grilla (M1) mejorado
            g2.setColor(new Color(100, 200, 255));
            g2.setFont(new Font("SansSerif", Font.BOLD, 18));
            g2.drawString("Misión 1: Rescate de Nina (" + nodesOrRows + "x" + edgesOrCols + ")", 20, 40);
            
            int cellSize = Math.min((w - 40) / edgesOrCols, (h - 60) / nodesOrRows);
            if (cellSize < 2) cellSize = 2;
            
            int startX = (w - (edgesOrCols * cellSize)) / 2;
            int startY = (h - (nodesOrRows * cellSize)) / 2 + 20;
            ...
            for (int r = 0; r < nodesOrRows; r++) {
                for (int c = 0; c < edgesOrCols; c++) {
                    ...
                    if (mines != null && mines[r][c]) {
                        ...
                        g2.fillRect(px, py, cellSize, cellSize);
                        // Dibujar 'X' sobre las minas si hay espacio
                        if (cellSize > 10) {
            ...
            g2.setFont(new Font("SansSerif", Font.BOLD, 18));
            g2.drawString("Grafo: " + title, 20, 40);
